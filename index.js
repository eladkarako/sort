...
//https://app.unpkg.com/json5@2.2.3/files/dist/index.js
//https://app.unpkg.com/json5@2.2.3/files/dist/index.min.js
...
function file_open_change_handler(ev){
  ...
  (async()=>{
    const s = await opener.files[0].text();
    set_textarea_value( s );
    reset_selection_focus();
  })();
...
function sort_and_unique_array_of_strings(arr){
  return arr.filter(s=>("string" === typeof s  &&  s.length > 0))
...
function sort_and_unique_textarea(inline_sort_separator){
  ...
  if("string" === typeof inline_sort_separator){
    lines = lines.map(line=>{
              line = line.split(inline_sort_separator);
              line = sort_and_unique_array_of_strings(line);
              line = line.join(inline_sort_separator);
              return line;
            });
  }
...
function json_beautify(is_deep_sort){
  is_deep_sort = "boolean" === typeof is_deep_sort ? is_deep_sort : false;
...
function save_textarea_to_file(){
  var date = new Date();
...
function key_handler(ev){
  ...
  switch (ev.keyCode) {
    case keycodes.DOM_VK_RETURN:  //Enter
                  console.log("sort_and_unique_textarea, without inline sorting");
                  sort_and_unique_textarea();
                  break;
    case keycodes.DOM_VK_COMMA:  //Comma
                  console.log("sort_and_unique_textarea, with inline sorting using \",\" as a separator.");
                  sort_and_unique_textarea(",");
                  break;
    case keycodes.DOM_VK_SLASH:  //Slash
                  console.log("sort_and_unique_textarea, with inline sorting using \" \" as a separator.");
                  sort_and_unique_textarea(" ");
                  break;
    case keycodes.DOM_VK_O:  //KeyO
                  console.log("opener");
                  opener.type  = "";
                  opener.value = "";
                  opener.type  = "file"
    ...
    case keycodes.DOM_VK_S:  //KeyS
                  ...
                  break;
    case keycodes.DOM_VK_J:  //KeyJ
                  console.log("json_beautify without deep sort");
                  json_beautify(false);
                  break;
    case keycodes.DOM_VK_K:  //KeyK
                  console.log("json_beautify with deep sort");
                  json_beautify(true);
                  break;
    case keycodes.DOM_VK_L:  //KeyL
                  console.log("cleaning textarea");
                  set_textarea_value("");
                  break;
    case keycodes.DOM_VK_W:  //KeyW
                  console.log("textarea toggling class to visually show all line in screen.");
                  textarea.classList.toggle("wrap-lines-ON");
                  break;
    default:
                  console.log("key code not register for any action.");
                  break;
  }
...
const button_click_hook = (ev)=>{
  console.log(ev);
  const button = ev.target || ev.srcElement;
  const title = button.getAttribute("title") || "";

  switch (title) {
    case "ctrl/alt+enter": //keycodes.DOM_VK_RETURN:  //Enter
      console.log("sort_and_unique_textarea, without inline sorting");
      sort_and_unique_textarea();
      break;
    case "ctrl/alt+,": //keycodes.DOM_VK_COMMA:  //Comma
      console.log("sort_and_unique_textarea, with inline sorting using \",\" as a separator.");
      sort_and_unique_textarea(",");
      break;
    case "ctrl/alt+/": //keycodes.DOM_VK_SLASH:  //Slash
      console.log("sort_and_unique_textarea, with inline sorting using \" \" as a separator.");
      sort_and_unique_textarea(" ");
      break;
    case "ctrl/alt+j": //keycodes.DOM_VK_J:  //KeyJ
      console.log("json_beautify without deep sort");
      json_beautify(false);
      break;
    case "ctrl/alt+k": //keycodes.DOM_VK_K:  //KeyK
      console.log("json_beautify with deep sort");
      json_beautify(true);
      break;
    case "ctrl/alt+l": //keycodes.DOM_VK_L:  //KeyL
      console.log("cleaning textarea");
      set_textarea_value("");
      break;
    case "ctrl/alt+w": //keycodes.DOM_VK_W:  //KeyW
      console.log("textarea toggling class to visually show all line in screen.");
      textarea.classList.toggle("wrap-lines-ON");
      break;
    case "ctrl/alt+o": //keycodes.DOM_VK_O:  //KeyO
      console.log("opener");
      opener.type  = "";
      opener.value = "";
      opener.type  = "file"
    ...
    case "ctrl/alt+s": //keycodes.DOM_VK_S:  //KeyS
      ...
      save_textarea_to_file();
      break;
    case "add tab character":
      console.log("add tab character");
      self.document.execCommand("insertText", false, "\t");
      break;
    case "to base64":
      console.log("to base64");
      set_textarea_value( base64_encode( textarea.value ) );
      break;
    case "from base64":
      console.log("from base64");
      set_textarea_value( base64_decode( textarea.value ) );
      break;
    default:
      console.log("button that has not been set an action was clicked");
  }  
};
...
buttons.forEach(button=>{
  button.addEventListener("click",  button_click_hook , {capture:false, passive:true, once:false});
});
